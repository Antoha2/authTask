...
func (s *AuthService) CreateUser(user *helper.User, userRoles *helper.UsersRoles) (int, error) {

	user.Password = s.generatePasswordHash(user.Password)
	userId, err := s.authRep.CreateUser(user, userRoles)
	if err != nil {
		fmt.Println(err)
		return 0, err
	}
...
func (s *AuthService) UpdateUser(user *helper.User) error {
