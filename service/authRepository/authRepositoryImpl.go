...
func (r *AuthPostgres) CreateUser(user *helper.User, userRoles *helper.UsersRoles) (int, error) {
	...
	//var id int
	roles := new(helper.Roles)
	//links := []etodo.UserlistToRoles{}
	links := helper.UserlistToRoles{UserId: user.UserId, RoleId: roles.RoleId}
...
func (r *AuthPostgres) GetUser(username, password string) (*helper.User, error) {
...
func (r *AuthPostgres) UpdateUser(user *helper.User) error {
...
func (r *AuthPostgres) GetRoles(userId int) []string {
	...
	result := r.dbx.Model(&helper.Roles{}).Select("roles.rolesname").
		Joins("INNER JOIN userlist_to_roles ON userlist_to_roles.user_id = ? AND userlist_to_roles.role_id = roles.role_id", userId).
		Scan(&roles)
